...
- Session Snapshot Command Placement is allowed only as a specification in `docs/session_snapshot_command_placement_v0_1.md`. It does not authorize implementation. Future save command direction must be V12-style restart checkpoint first, not Quest Snapshot auto-generation. Runtime expansion, `tools/save_session_snapshot.py`, `outputs/session_snapshot.md`, `outputs/quest_snapshot.md`, CLI behavior changes, PDF/PNG/screenshot automation, snapshot comparison, scoring, drift scoring, market research, external APIs, Gate upgrades, and external posting remain HOLD until a future Gate explicitly changes them.
